      ...
      <Collapse collapsible='header' ghost defaultActiveKey={['mapStyle', 'defaultZoom', 'defaultCenter']}>
        <Collapse.Panel header='地图样式' key='mapStyle' >
          <Space direction='vertical'>
            <Radio.Group onChange={(obj)=> {
              props.onChange(obj.target.value)
            }} defaultValue={props.mapStyle ? props.mapStyle : 'amap://styles/5228dc9ed756d11492490a95ae04ea3f'}>
            ...
            <Button block onClick={()=>{props.setDefaultStyle('dark')}}>设为默认</Button>
          </Space>
        </Collapse.Panel>
        <Collapse.Panel header='初始缩放层级' key='defaultZoom' >
          <Space direction='vertical'>
            <InputNumber addonBefore="缩放层级" value={props.mapZoom} disabled/>
            <Button block onClick={props.setDefaultZoom}>设为默认</Button>
          </Space>
        </Collapse.Panel>
        <Collapse.Panel header='初始坐标' key='defaultCenter' >
          <Space direction='vertical'>
            ...
            <Button block onClick={props.setDefaultCenter}>设为默认</Button>
          </Space>
        </Collapse.Panel>
      </Collapse>
    </Drawer>
